...
```
Search for files: package.json, pyproject.toml, requirements.txt, go.mod, Cargo.toml,
...
```
Search for content: "listen(", "createServer", "app.run", "uvicorn.run", "if __name__",
...
**Python:**
```
Search for content: "@app\.(get|post|put|patch|delete)", "@router\.", "@api_view",
      "path(", "url(", "@app\.route"
```

**TypeScript:**
```
Search for content: "router\.(get|post|put|patch|delete)", "@Get(", "@Post(", "@Put(",
...
```
Search for content: "class.*Model", "class.*Entity", "@Entity", "Base.metadata",
...
```
Search for content: "fetch(", "axios", "httpClient", "requests\.(get|post)",
...
```
Search for content: "emit(", "publish(", "dispatch(", "on(", "subscribe(",
...
```
Search for content: @(Get|Post|Put|Delete|Patch)\(|router\.(get|post|put|delete)|@app\.(get|post|put|delete|route)
```
...
```
Search for content: class.*Model|class.*Entity|@Entity|Table\(|Schema\(\{|model\s+\w+\s*\{
```
...
```
Search for content: fetch\(|axios\.|httpClient\.|requests\.(get|post)|Redis\(|S3\(|BlobServiceClient|sendgrid|stripe|twilio|kafka|amqp|SQS|SNS
```
