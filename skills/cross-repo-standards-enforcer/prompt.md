...
```
Search for files: .org-standards.yml, .org-standards.json, docs/org-standards.yml, .github/org-standards.yml
```
...
```
Search for files: .eslintrc*, .prettierrc*, .pylintrc, pyproject.toml, .ruff.toml, .rubocop.yml, .editorconfig
```
...
```
Search for files: azure-pipelines*.yml, .azure-pipelines/**/*.yml, .github/workflows/*.yml, .gitlab-ci.yml, Jenkinsfile
```
...
```
Search for content: "template:", "uses:", "extends:"
```
