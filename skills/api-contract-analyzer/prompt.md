...
```
Search for files: **/openapi.{yaml,yml,json}, **/swagger.{yaml,yml,json}, **/api-spec*.{yaml,yml,json}
```
...
**Express/Node.js:**
```
Search for content: "router\.(get|post|put|patch|delete|all)\s*\("
Search for content: "app\.(get|post|put|patch|delete)\s*\("
```

**FastAPI/Python:**
```
Search for content: "@(app|router)\.(get|post|put|patch|delete)\s*\("
```

**ASP.NET/C#:**
```
Search for content: "\[(HttpGet|HttpPost|HttpPut|HttpPatch|HttpDelete|Route)\("
Search for content: "\[ApiController\]"
```

**Spring/Java:**
```
Search for content: "@(GetMapping|PostMapping|PutMapping|PatchMapping|DeleteMapping|RequestMapping)\("
```

### GraphQL Schemas
```
Search for files: **/*.graphql, **/schema.{graphql,gql}, **/*.gql
Search for content: "type Query", "type Mutation", "type Subscription"
```

### gRPC Proto Files
```
Search for files: **/*.proto
Search for content: "service\s+\w+\s*\{", "rpc\s+\w+"
```
