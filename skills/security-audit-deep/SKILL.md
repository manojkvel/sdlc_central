...
description: Deep security audit for release gates and monthly compliance — IaC, container image, SBOM, CVSS, compliance frameworks, threat model. Use /security-audit for fast per-PR scans.
allowed-tools: Read, Write, Grep, Glob, Bash(git diff, git log, git show, npm audit, pip audit, cargo audit, bundle audit, govulncheck, mvn, semgrep, bandit, gosec, brakeman, gitleaks, trufflehog, tfsec, checkov, kubesec, trivy, grype, syft, docker, testssl.sh, nmap, curl, ls, mkdir, find, xargs, awk, date)
---
